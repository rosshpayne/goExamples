...
Source file src/net/http/server.go
  ...
  type writerOnly struct {
  	io.Writer                                                             // only io.Writer method promoted from concete type - no other methods available.
									      // this way we force only those methods from the concrete type we want to use.
  }
  
  func srcIsRegularFile(src io.Reader) (isRegular bool, err error) {
  	switch v := src.(type) {                                         // determine the dynamic type of the src interface ie. the concrete type in use.
  	case *os.File: 							 // either a *os.File or LimitedReader
  		fi, err := v.Stat()    					 // note: return variables are used - declared in return clause.
  									 // isRegular file default value is False - zero value of bool
  	...
  	default:
  		return                                                    // retuns default values False, nil
  	}
  }
  ...
  func (w *response) ReadFrom(src io.Reader) (n int64, err error) {
  	...
  	rf, ok := w.conn.rwc.(io.ReaderFrom)                              // check if rwc - ReaderWriterCloser interface's concrete type satisfies ReadFrom interface
  	regFile, err := srcIsRegularFile(src)
  	if err != nil {
  		return 0, err
  	}
  	if !ok || !regFile {
  		bufp := copyBufPool.Get().(*[]byte)                 // Get returns an interface{} - use TA (TypeAssertion) to check concrete type is *[]byte
								    // If sync Pool gets a nil it executes New function which allocates a buf & returns pointer to it.
  		defer copyBufPool.Put(bufp)
  		return io.CopyBuffer(writerOnly{w}, src, *bufp)     // pass pointer stored in Pool to []byte. i
    								    // We store pointer to buf in sync.Pool - so they are not unallocated by GC. 
 								    // Pointer to buf in heap means buf will not be deallocated by GC.  Pointer stored in sync.Pool will 
								    // not be deallocated as Pool no controlled by GC
  	}							    // struct writeOnly{w} exposes/promotes Write method only of concrete type w.
  ...
  var copyBufPool = sync.Pool{
  	New: func() interface{} {                             // return interface{} - meaning anyhting.  Must use Type Assertion to get the concrete value
  		b := make([]byte, 32*1024)
  		return &b                                     // return pointer to []byte slice ??? - surprised. makes sense as pointer types are ideal to persist to a sync Pool
  	},
  }
